...
export class RingRestClient {
  ...
  private extractCsrfToken(html: string, cookieJar: SimpleCookieJar): string {
    ...
    if (nextDataMatch) {
      try {
        const nextData = JSON.parse(nextDataMatch[1]),
          csrfToken = this.findCsrfInObject(nextData)
    ...
    logDebug('CSRF extraction failed. Cookies: ' + cookieJar.getCookieHeader())
    logDebug('HTML preview (first 2000 chars): ' + html.substring(0, 2000))
  ...
  private async getAuthorizationCode(): Promise<string> {
    ...
    logDebug(`Authorize code request cookies: ${cookieJar.getCookieHeader()}`)
    ...
    for (let i = 0; i < 5; i++) {
      ...
      logDebug(
        `Authorize non-redirect response (${response.status}): ${body.substring(
          0,
          1000,
        )}`,
      )
